...
class _FakeImage:
    ...
    def __init__(self, height=1023, width=1920):
        self.height = height
        self.width = width
...
def _mock_enter_quantity(
    ...
    monkeypatch.setattr(
        adapter, "_shifted_anchor",
        lambda name, offset: (0, state["row_top"] - adapter._pending_quantity_target.after_index * _GRID_ROW_HEIGHT, 0, 0),
    )
    ...
    monkeypatch.setattr(
        adapter, "_read_quantity_at",
        lambda image, offset, row_top, **kwargs: (
            typed.get("value", 0.0) + confirmed_qty_offset
            if state["tabbed"] else existing_qty_at_target
        ),
    )
...
def _wire_identity_quantity_flow(
    monkeypatch, adapter, rows, quantities_before, expected, *,
    rows_after=None, confirmed_quantity=..., row_count_after=None, target_disappears=False,
):
    ...
    monkeypatch.setattr(adapter, "_ensure_main_window", lambda: 123)
    monkeypatch.setattr(adapter, "_capture_and_locate", lambda *args, **kwargs: (image, (0, 0)))
    def geometry(image, offset):
        if state["typed"] and target_disappears:
            return None
        count = row_count_after if state["typed"] and row_count_after is not None else len(rows)
        return (count, row_1_top + (count - 1) * 25) if count else None

    monkeypatch.setattr(adapter, "_last_row_geometry", geometry)
    ...
    def read_row(image, offset, row_top):
        ...
        active_rows = rows_after if state["typed"] and rows_after is not None else rows
        return active_rows[index] if 0 <= index < len(active_rows) else ActivationRowSnapshot(None, None, None, None)
...
def test_same_cell_quantity_confirmation_ignores_noisy_post_edit_cat_ocr(monkeypatch):
    adapter = WindowsXactimateAdapter(expected_project_name="TEST", window_finder=lambda: ([], []))
    before = [ActivationRowSnapshot("SFG", "GUTA", "Aluminum gutter", None)]
    after = [ActivationRowSnapshot("zs", "GUTA", "Aluminum gutter", None)]
    adapter._pending_quantity_target = PendingQuantityTarget(("sfg", "guta", "aluminum gutter"), None, 0, 1)
    state, reads, _clicks = _wire_identity_quantity_flow(
        monkeypatch, adapter, before, [0.0], 200.0, rows_after=after,
    )

    adapter.enter_quantity(200.0)

    assert state["clicked_index"] == 0
    assert reads == [0, 0]


def test_same_cell_quantity_confirmation_ignores_noisy_post_edit_description(monkeypatch):
    adapter = WindowsXactimateAdapter(expected_project_name="TEST", window_finder=lambda: ([], []))
    before = [ActivationRowSnapshot("SFG", "GUTA", "Aluminum gutter", None)]
    after = [ActivationRowSnapshot("SFG", "GUTA", "unreadable repaint noise", None)]
    adapter._pending_quantity_target = PendingQuantityTarget(("sfg", "guta", "aluminum gutter"), None, 0, 1)
    _wire_identity_quantity_flow(monkeypatch, adapter, before, [0.0], 200.0, rows_after=after)
...
def test_same_cell_quantity_confirmation_rejects_row_count_change(monkeypatch):
    ...
    rows = [ActivationRowSnapshot("SFG", "GSG", "Gutter splash guard", None)]
    adapter._pending_quantity_target = PendingQuantityTarget(("sfg", "gsg", "gutter splash guard"), None, 0, 1)
    _wire_identity_quantity_flow(monkeypatch, adapter, rows, [0.0], 1.0, row_count_after=2)

    with pytest.raises(QuantityNotConfirmedError, match="row count changed unexpectedly"):
        adapter.enter_quantity(1.0)


def test_same_cell_quantity_confirmation_rejects_disappeared_target(monkeypatch):
    adapter = WindowsXactimateAdapter(expected_project_name="TEST", window_finder=lambda: ([], []))
    rows = [ActivationRowSnapshot("SFG", "GSG", "Gutter splash guard", None)]
    adapter._pending_quantity_target = PendingQuantityTarget(("sfg", "gsg", "gutter splash guard"), None, 0, 1)
    _wire_identity_quantity_flow(monkeypatch, adapter, rows, [0.0], 1.0, target_disappears=True)

    with pytest.raises(QuantityNotConfirmedError, match="target row disappeared"):
        adapter.enter_quantity(1.0)


def test_previous_unrelated_quantity_cannot_satisfy_same_cell_confirmation(monkeypatch):
    adapter = WindowsXactimateAdapter(expected_project_name="TEST", window_finder=lambda: ([], []))
    rows = [
        ActivationRowSnapshot("SFG", "GUTA", "Aluminum gutter", None),
        ActivationRowSnapshot("SFG", "GSG", "Gutter splash guard", None),
...
def test_prepopulated_exact_quantity_remains_write_free_with_same_cell_model(monkeypatch):
    ...
    rows = [ActivationRowSnapshot("SFG", "GSG", "Gutter splash guard", None)]
    adapter._pending_quantity_target = PendingQuantityTarget(("sfg", "gsg", "gutter splash guard"), None, 0, 1)
    state, reads, clicks = _wire_identity_quantity_flow(monkeypatch, adapter, rows, [1.0], 1.0)
...
def test_rr_plus_row_uses_prebound_same_cell_after_noisy_repaint(monkeypatch):
    adapter = WindowsXactimateAdapter(expected_project_name="TEST", window_finder=lambda: ([], []))
    before = [_activation_row(activity="-"), _activation_row(activity="+")]
    after = [_activation_row(activity="-"), ActivationRowSnapshot("??", "STEEP", "noise", "o")]
    adapter._pending_quantity_target = PendingQuantityTarget(("rfg", "steep", "steep charge"), "+", 1, 1)
    state, reads, _clicks = _wire_identity_quantity_flow(
        monkeypatch, adapter, before, [0.0, 0.0], 33.66, rows_after=after,
    )
...
def _script_rr_binding_frames(monkeypatch, adapter, frames):
    observations = iter(frames)
    calls = []

    def snapshot():
        rows = next(observations)
        calls.append(rows)
        return rows
...
def test_rr_binding_retries_unreadable_first_frame_and_binds_valid_second_frame(monkeypatch):
    adapter = WindowsXactimateAdapter(expected_project_name="TEST", window_finder=lambda: ([], []))
    unreadable = [_rr_pair_row(activity="_L"), _rr_pair_row(activity="=")]
    valid = [_rr_pair_row(activity="-"), _rr_pair_row(activity="+")]
    calls = _script_rr_binding_frames(monkeypatch, adapter, [unreadable, valid])

    result = adapter.bind_rr_pair_after_activation([], timeout_s=8)

    assert len(calls) == 2
    assert result.minus_target.after_index == 0
    assert result.plus_target.after_index == 1


def test_rr_binding_allows_two_unreadable_frames_then_binds_third(monkeypatch):
    adapter = WindowsXactimateAdapter(expected_project_name="TEST", window_finder=lambda: ([], []))
    unreadable = [_rr_pair_row(activity="_L"), _rr_pair_row(activity="=")]
    valid = [_rr_pair_row(activity="-"), _rr_pair_row(activity="+")]
    calls = _script_rr_binding_frames(monkeypatch, adapter, [unreadable, unreadable, valid])
...
def test_rr_binding_three_unreadable_frames_fail_task_locally(monkeypatch):
    adapter = WindowsXactimateAdapter(expected_project_name="TEST", window_finder=lambda: ([], []))
    unreadable = [_rr_pair_row(activity="_L"), _rr_pair_row(activity="=")]
    calls = _script_rr_binding_frames(monkeypatch, adapter, [unreadable, unreadable, unreadable])

    with pytest.raises(TaskLocalRowReconciliationError, match="uniquely bound R&R"):
        adapter.bind_rr_pair_after_activation([], timeout_s=8)
...
def test_rr_binding_delta_change_during_role_grace_fails_closed(monkeypatch, changed_frame):
    adapter = WindowsXactimateAdapter(expected_project_name="TEST", window_finder=lambda: ([], []))
    unreadable = [_rr_pair_row(activity="_L"), _rr_pair_row(activity="=")]
    calls = _script_rr_binding_frames(monkeypatch, adapter, [unreadable, changed_frame])

    with pytest.raises(PhysicalStateUncertainError, match="changed while waiting"):
        adapter.bind_rr_pair_after_activation([], timeout_s=8)

    assert len(calls) == 2


def test_rr_binding_identity_change_during_role_grace_fails_closed(monkeypatch):
    adapter = WindowsXactimateAdapter(expected_project_name="TEST", window_finder=lambda: ([], []))
    unreadable = [_rr_pair_row(activity="_L"), _rr_pair_row(activity="=")]
    different_identity = [
        ActivationRowSnapshot("RFG", "HIGH", "Additional charge for high roof", "-"),
        ActivationRowSnapshot("RFG", "HIGH", "Additional charge for high roof", "+"),
    ]
    calls = _script_rr_binding_frames(monkeypatch, adapter, [unreadable, different_identity])

    with pytest.raises(PhysicalStateUncertainError, match="identity changed"):
        adapter.bind_rr_pair_after_activation([], timeout_s=8)

    assert len(calls) == 2


def test_rr_binding_baseline_change_during_role_grace_fails_closed(monkeypatch):
    adapter = WindowsXactimateAdapter(expected_project_name="TEST", window_finder=lambda: ([], []))
    baseline = [ActivationRowSnapshot("SFG", "GUTA", "Aluminum gutter", None)]
    adapter._last_activation_baseline_rows = baseline
    unreadable = baseline + [_rr_pair_row(activity="_L"), _rr_pair_row(activity="=")]
    changed_baseline = [
        ActivationRowSnapshot("PLM", "TLT", "Toilet", None),
        _rr_pair_row(activity="-"),
        _rr_pair_row(activity="+"),
    ]
    calls = _script_rr_binding_frames(monkeypatch, adapter, [unreadable, changed_baseline])

    with pytest.raises(PhysicalStateUncertainError, match="structure changed"):
        adapter.bind_rr_pair_after_activation([("SFG", "GUTA")], timeout_s=8)

    assert len(calls) == 2


def test_rr_binding_dialog_on_readable_retry_frame_fails_before_binding(monkeypatch):
    adapter = WindowsXactimateAdapter(expected_project_name="TEST", window_finder=lambda: ([], []))
    unreadable = [_rr_pair_row(activity="_L"), _rr_pair_row(activity="=")]
    valid = [_rr_pair_row(activity="-"), _rr_pair_row(activity="+")]
    calls = _script_rr_binding_frames(monkeypatch, adapter, [unreadable, valid])
    dialog_states = iter([False, True])
    monkeypatch.setattr(adapter, "_unexpected_dialog_present", lambda: next(dialog_states))

    with pytest.raises(PhysicalStateUncertainError, match="unexpected dialog"):
        adapter.bind_rr_pair_after_activation([], timeout_s=8)

    assert len(calls) == 2


def test_rr_binding_reversed_retry_frame_binds_roles_not_row_order(monkeypatch):
    adapter = WindowsXactimateAdapter(expected_project_name="TEST", window_finder=lambda: ([], []))
    unreadable = [_rr_pair_row(activity="_L"), _rr_pair_row(activity="=")]
    reversed_valid = [_rr_pair_row(activity="+"), _rr_pair_row(activity="-")]
    calls = _script_rr_binding_frames(monkeypatch, adapter, [unreadable, reversed_valid])

    result = adapter.bind_rr_pair_after_activation([], timeout_s=8)

    assert len(calls) == 2
    assert result.minus_target.after_index == 1
    assert result.plus_target.after_index == 0


@pytest.mark.parametrize("activities", [("-", "+"), ("-", "4"), ("to", "+")])
def test_existing_rr_activity_paths_bind_first_frame_without_grace(monkeypatch, activities):
    adapter = WindowsXactimateAdapter(expected_project_name="TEST", window_finder=lambda: ([], []))
    frame = [_rr_pair_row(activity=activity) for activity in activities]
    calls = _script_rr_binding_frames(monkeypatch, adapter, [frame])
...
def test_ordinary_activation_does_not_use_rr_activity_render_grace(monkeypatch):
    adapter = WindowsXactimateAdapter(expected_project_name="TEST", window_finder=lambda: ([], []))
    ordinary = [ActivationRowSnapshot("TMP", "LAB", "Temporary repairs - per hour", None)]
    calls = []
    ...
    assert len(calls) == 1
    assert adapter._pending_quantity_target.identity == (
        "tmp", "lab", "temporary repairs per hour",
    )
